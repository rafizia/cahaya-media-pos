...
export const CartLedger: React.FC<CartLedgerProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-xl border border-neutral-200 overflow-hidden shadow-xs">
      ...
      <div className="p-3.5 bg-pos-dark border-b border-[#082E47] flex items-center gap-2.5">
        <div className="flex items-center gap-1.5 shrink-0">
          <KeyBadge shortcut="F1" variant="blue" />
          <span className="text-xs font-bold text-blue-100 uppercase tracking-wider hidden sm:inline">
            Scan
          </span>
        </div>
        <div className="relative flex-1">
          <input
            ...
            className="w-full pl-3 pr-20 py-2 bg-[#082E47]/90 text-white placeholder-blue-200/50 rounded-lg text-sm font-mono border border-[#0F4C75]/60 outline-none focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400 transition-all"
            autoFocus
          />
        ...
        {cart.length > 0 && onClearCart && (
          <button
            onClick={onClearCart}
            className="px-2.5 py-2 text-xs font-semibold text-blue-200/70 hover:text-red-300 hover:bg-[#082E47] rounded-lg transition-colors cursor-pointer"
      ...
      <div className="px-4 py-2.5 bg-neutral-50 border-b border-neutral-100 flex items-center justify-between text-xs text-neutral-600 font-medium">
        <span>
          Struk Aktif:{" "}
        ...
        <span>
          Total:{" "}
          <strong className="text-neutral-900 font-bold font-mono text-sm">
            {totalItemCount} Pcs
          </strong>
        </span>
      </div>
          ...
          <tbody className="divide-y divide-neutral-100 text-sm font-mono tabular-nums">
            {cart.map((item, idx) => {
              ...
              return (
                ...
                >
                  <td className="py-3 px-3 text-center text-xs text-neutral-400">
                  ...
                  <td className="py-3 px-3 text-center">
                    <div className="inline-flex items-center border border-neutral-200 rounded-lg bg-neutral-50 p-0.5 shadow-2xs">
                      <button
                        ...
                        className="w-6 h-6 flex items-center justify-center text-neutral-600 hover:bg-neutral-200 hover:text-neutral-900 rounded font-bold text-sm cursor-pointer transition-colors"
                        title="Kurangi 1"
                      >
                        -
                      </button>
                      <input
                        ...
                        className="w-9 text-center text-sm font-bold text-neutral-800 bg-transparent outline-none p-0 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                      />
                      <button
                        type="button"
                        onClick={() =>
                  ...
                  <td className="py-3 px-3 text-right text-neutral-600 text-sm">
