  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-xl border border-neutral-200 overflow-hidden shadow-xs">
      ...
      <div className="p-3.5 bg-neutral-50 border-b border-neutral-200 flex flex-col sm:flex-row gap-2.5 items-stretch sm:items-center justify-between">
        <div className="relative flex-1">
          <input
            ...
            className="w-full pl-3 pr-8 py-2 bg-white text-neutral-900 placeholder-neutral-400 rounded-lg text-xs font-medium border border-neutral-300 outline-none focus:border-[#0F62FE] transition-all"
          />
          {search && (
            <button
              onClick={() => setSearch("")}
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className="px-3 py-2 bg-white text-neutral-800 rounded-lg text-xs font-medium border border-neutral-300 outline-none focus:border-[#0F62FE] cursor-pointer"
          >
            {categories.map((c) => (
          ...
          {lowStockCount > 0 && (
            <div className="text-[11px] font-bold text-amber-800 bg-amber-50 border border-amber-200 px-2.5 py-1.5 rounded-lg whitespace-nowrap">
              ...
              return (
                ...
                >
                  <td className="py-2.5 px-3">
                    <div className="font-semibold text-neutral-900 text-xs group-hover:text-[#0F62FE] transition-colors">
                      {p.name}
                    </div>
                  ...
                  <td className="py-2.5 px-2 text-center font-mono tabular-nums">
                    <div className="font-bold text-xs text-neutral-800">{p.stock}</div>
                    {isLowStock ? (
                  ...
                  <td className="py-2.5 px-3 text-right font-mono text-xs text-neutral-500 tabular-nums">
                    {(p.cost_price ?? 0).toLocaleString("id-ID")}
                  </td>
                  <td className="py-2.5 px-3 text-right font-mono font-bold text-xs text-neutral-900 tabular-nums">
      ...
      <div className="px-3.5 py-2 bg-neutral-50 border-t border-neutral-200 text-xs text-neutral-500 font-mono flex items-center justify-between">
        <span>Total: <strong>{products.length} Produk</strong></span>
        <span>Filter: <strong>{filteredProducts.length} Ditampilkan</strong></span>
      </div>
    </div>
  );
};
